...
// Creates and saves a new puzzle using input from the user
...
public:
    PuzzleGenerator();
    void createWordSearch(std::ofstream& out, std::ifstream& in);
    void createEmptyPuzzle(int& wordCount, int &maxRows, int &maxCols, char wordSearchLetters[][1000]);
    void fillWordSearch(int maxRows, int maxCols, char wordSearchLetters[][1000],
                        std::string& word);
